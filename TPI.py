...
def solve_tp(g_n_path, omega_S_preTP, rho_s, imm_rates_path, params):
    ...
    # Put defining paths out of loop
    b_11 = 1.1 * b_ss
    ...
    rpath_init[:T] = np.linspace(r_11, r_ss, T)
    rpath_init[T:] = r_ss
    ...
    while dist > mindist and tpi_iter < maxiter:
        ...
        # dist is the deviations for the BQ path and the r path.
        dist = ((BQpath_init[:T] - new_BQpath[:T]) ** 2).sum()+ ((rpath_init[:T] - new_rpath[:T]) ** 2).sum()
    ...
    return [new_Kpath, new_BQpath, new_rpath]
