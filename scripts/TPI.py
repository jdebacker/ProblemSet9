...
def HHFOCs(b_sp1, *params):
    ...
    # rpath = length p, n = length p, p = number of periods left in life time, wpath = length p, b_sp1 = length p-1
...
BQ_1 = get_BQ(b_1, r_1, BQ_params)
K_1 = get_K(b_1, K_params)
BQpath_init = np.zeros(T + S - 1)
Kpath_init = np.zeros(T + S - 1)
BQpath_init[:T] = np.linspace(BQ_1, BQ_ss, T)
Kpath_init[:T] = np.linspace(K_1, K_ss, T)
BQpath_init[T:] = BQ_ss
Kpath_init[T:] = K_ss
r_1 = get_r(K_1, L_ss, r_params)
